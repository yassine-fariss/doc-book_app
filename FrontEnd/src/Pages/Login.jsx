...
const Login = () => {
  ...
  useEffect(() => {
    const savedEmail = get("REMEMBER_ME_USER");
    if (savedEmail) {
    ...
    if (searchParams.get("demo") === "patient") {
      setDataForm({
  ...
  const HandleSubmit = (e) => {
    ...
    axiosClient
      ...
      .then(({ data }) => {
        ...
        if (rememberMe) {
          storeInLocalStorage("REMEMBER_ME_USER", DataForm.email);
        } else {
          ...
          <div className="w-full max-w-md bg-white rounded-3xl border border-gray-100 shadow-xl p-8 md:p-10 space-y-6 text-left animate-fade-in">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-xs md:text-sm text-gray-400 leading-relaxed max-w-xs mx-auto">
                Access your consultation portal, check schedule times, and book appointments.
              </p>
            </div>
            ...
            <div className="pt-4 border-t border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setDataForm({ email: "demo.patient@healthconnect.ma", password: "DemoPatient123!" })}
                  className="w-full py-2.5 bg-blue-50 text-blue-700 hover:bg-blue-100 font-bold rounded-xl text-xs transition duration-200 border border-blue-200"
                >
                  Patient Demo
                </button>
                <Link
                  to="/doctor/login?demo=doctor"
                  className="w-full py-2.5 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-bold rounded-xl text-xs transition duration-200 border border-indigo-200 flex justify-center items-center"
                >
                  Doctor Demo
                </Link>
              </div>
            ...
            <div className="border-t border-gray-50 pt-6 text-center space-y-3">
              <p className="text-xs md:text-sm text-gray-500">
                ...
                <Link to="/identifier" className="text-blue-600 font-bold hover:underline">
                  Sign Up
                </Link>
              </p>
